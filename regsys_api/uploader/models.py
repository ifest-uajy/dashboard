...
class PersonalUploadFile(models.Model):
    ...
    uploaded_by = models.ForeignKey(to=User, related_name='personal_uploaded_files', on_delete=models.CASCADE)
...
class TeamUploadFile(models.Model):
    ...
    file_size = models.BigIntegerField()
    content_type = models.CharField(max_length=200)
    team_owner = models.ForeignKey(to=HackathonTeams, related_name='team_owner', on_delete=models.PROTECT)
    uploaded_by = models.ForeignKey(to=User, related_name='team_uploaded_files', on_delete=models.PROTECT)
    uploaded_at = models.DateTimeField(auto_now_add=True)
