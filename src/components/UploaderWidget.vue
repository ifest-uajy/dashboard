  ...
  <v-flex>
    <v-form @submit.prevent="sumbitFile">
      <v-flex>
        ...
        <v-alert
          ...
          outlined
          prominent
          >File yang anda upload tidak sesuai dengan kriteria. Silahkan upload
          ulang atau hubungi pihak panitia untuk informasi lebih
          lanjut.</v-alert
        >
        <v-alert
          v-if="
            response.length !== 0 && response.status === 'menunggu_verifikasi'
          "
          :value="true"
          type="info"
          outlined
        ...
        >
          ...
          <br />Mohon tunggu dalam waktu <b>1 x 24</b> jam.
        </v-alert>
        <v-alert v-if="dropzoneError" :value="true" type="error" outlined>{{
          dropzoneError
        }}</v-alert>
      </v-flex>
    </v-form>
    <v-dialog v-model="dialog" persistent max-width="350">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" text @click="handleUpload"
            >Upload</v-btn
          >
    ...
    <v-snackbar v-model="uploadBatalSB">
