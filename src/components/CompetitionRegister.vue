    ...
    >
      ...
      >
        <v-container>
          ...
          <v-container class="px-0 grey--text text--darken-4 title"
            >Informasi Pendamping Tim</v-container
          >
          ...
          <p class="mr-1 ml-1 mt-5">
            ...
            <br />
            Dengan membuat tim saya bersedia untuk menaati segala peraturan yang
            telah dipublikasikan dalam laman resmi tiap kompetisi perlombaan.
          </p>
...
export default {
  ...
  computed: {
    ...
    instituteErrors() {
      ...
      !this.$v.namaInstitusi.maxLength &&
        errors.push("This field must be at most 100 characters long");
      !this.$v.namaInstitusi.required &&
        errors.push("Asal institusi diperlukan untuk pendaftaran tim.");
      return errors;
    },
    nameErrors() {
      const errors = [];
      ...
      !this.$v.name.minLength &&
        errors.push("Nama tim harus paling tidak terdiri dari 3 karakter.");
    ...
    addressErrors() {
      ...
      !this.$v.alamatInstitusi.required &&
        errors.push("Alamat asal institusi diperlukan untuk pendaftaran tim.");
      return errors;
    },
    pendampingNameErrors() {
      const errors = [];
      if (!this.$v.namaPendamping.$dirty) return errors;
      !this.$v.namaPendamping.required &&
        errors.push("Nama pendamping tim tidak boleh kosong.");
      return errors;
    },
    teleponPendampingErrors() {
      const errors = [];
      if (!this.$v.teleponPendamping.$dirty) return errors;
      !this.$v.teleponPendamping.required &&
        errors.push("Nomor telepon pendamping tim tidak boleh kosong.");
      !this.$v.teleponPendamping.maxLength &&
        errors.push(
          "Nomor telepon pendamping must be at most 15 characters long"
        );
      !this.$v.teleponPendamping.minLength &&
        errors.push(
          "Nomor telepon pendamping tim harus paling tidak terdiri dari 7 karakter."
        );
      return errors;
    },
    isComplete() {
      return (
        this.name &&
        this.alamatInstitusi &&
        this.namaInstitusi &&
        this.teleponPendamping &&
        this.namaPendamping &&
        this.teleponPendamping.length >= 7 &&
        this.name.length >= 3
      );
    }
  },
