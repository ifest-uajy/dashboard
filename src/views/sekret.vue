    ...
    <v-container justify-center class="boxed-isi">
      <v-card outlined width="1700" class="card_cloverleaf mb-5 pt-0 mt-10">
        <v-card-subtitle class="mt-5 pb-0 mb-0">
          <router-link to="/administrasi" class="link_clover">Kembali</router-link>
        </v-card-subtitle>

        <div v-if="loading">
          <v-skeleton-loader class="ml-4 mt-5 mb-2" type="heading" max-width="350"></v-skeleton-loader>
          <v-skeleton-loader class="ml-4 mt-5 mb-2" type="text" max-width="350"></v-skeleton-loader>
        </div>
        <div v-else>
          <v-card-title class="title_card_cloverleaf mt-0 pb-3">{{slug.name}}</v-card-title>
          <v-card-subtitle
            class="subtitle_card_cloverleaf mb-5 pt-2 pb-0"
              ...
              <div v-else>
                <v-dialog v-model="dialog" max-width="1200px">
                  <v-card outlined class="card_cloverleaf pt-5">
                    <v-card-title style="font-size: 20pt">Data Detail Tim</v-card-title>
                    <v-card-text>
                      <!-- <v-container> -->
                      <v-row>
                        <v-col>
                        ...
                        <v-col>
                          ...
                          <p class="subtitle-outer">{{detail.pembimbing.nama}}</p>
                          <span class="subtitle-inner">Kontak Pendamping</span>
                          <p class="subtitle-outer">(+62) {{detail.pembimbing.telepon}}</p>
                          ...
                          <p
                            class="subtitle-outer"
                          >{{moment(String(detail.created_at)).format("DD MMMM YYYY hh:mm A ")}}</p>
                        </v-col>
                      </v-row>
                      <!-- </v-container> -->
                      ...
                      <v-simple-table>
                        <template v-slot:default>
                          ...
                          <tbody>
                            <tr v-for="(item, key) in detail.anggota" :key="key">
                              <td>{{key+1}}</td>
                              ...
                              <td>{{ item.email }}</td>
                              <td>{{ item.nomor_id}}</td>
                              <td>{{ moment(String(item.tanggal_lahir)).format("DD MMM YYYY")}}</td>
                      ...
                      <p class="subtitle-outer">{{detail.current_task.name}}</p>
                        ...
                        <template v-slot:default>
                          ...
                          <tbody>
                            <tr v-for="(item, key) in detail.tasks" :key="key">
                              ...
                              <td>
                                <span v-if="item.response.status==='selesai'">Selesai</span>
                                <span
                                  v-if="item.response.status==='menunggu_verifikasi'"
                                >Menunggu Verifikasi</span>
                                <span v-if="item.response.status==='ditolak'">Ditolak</span>
                                <span v-if="!item.response.status">Belum ada respon</span>
                              </td>
                              <td>
                                <span
                                  v-if="item.task.task_type!=='pengumuman' && item.task.id <= detail.current_task.id && item.response.length !== 0"
                                >
                                  <a
                                    :href="`/api/file/download/` + item.response.response +`/`"
                                    class="body-link"
                                    target="_blank"
                                  >
                                    <v-btn depressed color="primary" small>Unduh Respon</v-btn>
                                  </a>
                                </span>
                                <span v-else>-</span>
                              </td>
                              <td>
                                <span v-if="item.response.status==='menunggu_verifikasi'">
                                  <v-btn
                                    depressed
                              ...
                              <td class="text-center">
                                ...
                                <span
                                  v-if="!item.response.status || item.response.status==='menunggu_verifikasi'"
                                >
                                  <v-icon color="warning">mdi-help-circle</v-icon>
                                </span>

                                <span
                                  v-if="item.response.is_verified===false && item.response.status!=='menunggu_verifikasi'"
                                >
                                  <v-icon color="red">mdi-close-circle</v-icon>
                                </span>
                              </td>
                              <td
                                v-if="item.response.updated_at"
                              >{{moment(String(item.response.updated_at)).format("DD MMMM YYYY hh:mm A ")}}</td>
                              <td v-else>-</td>
                            </tr>
                          </tbody>
                        </template>
                    ...
                    <v-card-text>
                      ...
                      <p>
                        <v-icon color="green">mdi-check-circle</v-icon>Berarti sudah di verifikasi baik oleh PH untuk pembayaran atau Sekretariat untuk task administrasi.
                      </p>
                      <p>
                        <v-icon color="warning">mdi-help-circle</v-icon>Menunggu di verifikasi atau belum ada respon dari tim.
                      </p>
                      <p>
                        <v-icon color="red">mdi-close-circle</v-icon>Task ditolak oleh panitia dan menunggu tim mengupload ulang atau menyerah.
                      </p>
                      <p class="mt-2">
                        <strong>PH hanya bisa memverifikasi task Pembayaran Registrasi. Sekretariat hanya bisa memverifikasi task yang perlu diverifikasi selain Pembayaran Registrasi.</strong>
                      </p>
                      <p>
                        Note buat verifikator,
                        <span
                          style="font-weight:bold; color:red;"
                        >GUNAKAN TOMBOL DENGAN BIJAK</span>. There's no way back after clicking yet.
                      </p>
                    </v-card-text>
                  </v-card>
                </v-dialog>
